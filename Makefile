...
DESCRIPTION: DESCRIPTION.in .git/index
	cat DESCRIPTION.in | sed -e "s/VERSION/$(VERSION)/" > DESCRIPTION

.PHONY: git-status

R/MxVersion.R: R/MxVersion.R.in git-status
	sed -e s/GIT_VERSION/$$(git describe --dirty)/ R/MxVersion.R.in > $@

dev-doc: DESCRIPTION R/MxVersion.R
	-./util/rox
	...
	grep -l -F 'Generated by roxygen2' man/*.Rd | sort >> .gitignore.new
	mv .gitignore.new .gitignore

maybe-dev-doc: DESCRIPTION R/MxVersion.R
	-if ! grep -q 'Generated by roxygen2' man/*.Rd; then ./util/rox; fi

build-prep: dev-doc R/MxVersion.R
	mkdir -p build
...
cran-check: cran-build
	...
	@if [ $$(wc -l OpenMx.Rcheck/00check.log | cut -d ' ' -f 1) -gt 205 ]; then echo "CRAN check problems have grown; see cran-check.log" ; false; fi
