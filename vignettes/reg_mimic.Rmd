...
mimicModel <- mxModel(
  ...
  mxRegularizeLASSO(what=covariates, name="LASSO",
...
```{r}
plan <- omxDefaultComputePlan(mimicModel$name, regularize = TRUE)

# After regularizing, fix zerod parameters to zero, discard the
# regularization penalty, and re-fit model.
plan$steps$REG$fixZeros <- TRUE

regMIMIC <- mxRun(mxModel(mimicModel, plan))

detail <- regMIMIC$compute$steps$REG$output$detail
...
However, due to `fixZeros`, the final covariate estimates are either exactly zero or estimated without the regularization penalty.
