...
```{r,results='hide'}
regFit <- mxRun(mxModel(
  fit, mxRegularizeLASSO(paste0('c',1:8),"lasso",lambda.step=1.2),
...
detail <- regFit$compute$steps$REG$output$detail
...
In this final model, we set the regression paths for x1, x2, x3, x4, x5, and x6 to zero. We make a mistake for x6, but we also correctly identify x7 and x8 as true paths. Compare these results with the maximum likelihood estimates.
