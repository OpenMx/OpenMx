...
RegularizingPenalty::RegularizingPenalty(S4 obj)
{
  params = obj.slot("params");
  epsilon = obj.slot("epsilon");
  scale = obj.slot("scale");
...
void RegularizingPenalty::fixZeros(FitContext *fc)
{
  for (int px = 0; px < params.size(); ++px) {
    if (fabs(fc->est[params[px]] / scale[px % scale.size()]) <= epsilon[px % epsilon.size()]) {
      fc->est[params[px]] = 0;
      fc->profiledOutZ[params[px]] = true;
    }
  }
}

double RegularizingPenalty::penaltyStrength(double absPar, int px) const
{
...
LassoPenalty::LassoPenalty(S4 obj) : RegularizingPenalty(obj)
{
...
double LassoPenalty::eval(FitContext *fc)
{
  double tmp = 0;
  for (int px = 0; px < params.size(); ++px) {
  ...
  return tmp * fc->est[lambda];
}

void LassoPenalty::gradient(FitContext *fc)
{
...
RidgePenalty::RidgePenalty(S4 obj) : RegularizingPenalty(obj)
{
  IntegerVector pv = obj.slot("hyperparameters");
...
double RidgePenalty::eval(FitContext *fc)
{
  double tmp = 0;
  for (int px = 0; px < params.size(); ++px) {
...
void RidgePenalty::gradient(FitContext *fc)
{
...
ElasticNetPenalty::ElasticNetPenalty(S4 obj) : RegularizingPenalty(obj)
{
  IntegerVector pv = obj.slot("hyperparameters");
...
double ElasticNetPenalty::eval(FitContext *fc)
{
  double lasso = 0;
  double ridge = 0;
  for (int px = 0; px < params.size(); ++px) {
  ...
  return fc->est[lambda] * ((1-prop) * ridge + prop * lasso);
}

void ElasticNetPenalty::gradient(FitContext *fc)
{
  double prop = fc->est[ alpha ];
  double la = fc->est[lambda];
  for (int px = 0; px < params.size(); ++px) {
