...
struct forward_difference_grad_csolnp {
    ...
    void operator()(T3 ff, double refFit, int thrId, double *point, int nineq, int np, double offset, int px, int numIter, double *Gaprox, double *fullPointV, Eigen::MatrixBase<T2>& yy_e, double rho, Eigen::MatrixBase<T1>& vscale_e, Eigen::MatrixBase<T2>& a_e, Eigen::MatrixBase<T2>& b_e, bool flag)
    {
        double orig = point[px];
        for(int k = 0; k < numIter; k++) {
            point[px] = orig + offset;
            fullPointV[nineq + px] = orig + offset;
            Gaprox[k] = (ff(point, thrId, np, fullPointV, yy_e, rho, vscale_e, a_e, b_e, flag) - refFit) / offset;
            offset *= .5;
        }
        point[px] = orig;
        fullPointV[nineq + px] = orig;
    };
};
...
struct central_difference_grad_csolnp {
    ...
    void operator()(T3 ff, double refFit, int thrId, double *point, int nineq, int np, double offset, int px, int numIter, double *Gaprox, double *fullPointV, Eigen::MatrixBase<T2>& yy_e, double rho, Eigen::MatrixBase<T1>& vscale_e, Eigen::MatrixBase<T2>& a_e, Eigen::MatrixBase<T2>& b_e, bool flag)
    {
        ...
        for(int k = 0; k < numIter; k++) {
            point[px] = orig + offset;
            fullPointV[nineq + px] = orig + offset;
            double f1 = ff(point, thrId, np, fullPointV, yy_e, rho, vscale_e, a_e, b_e, flag);
            point[px] = orig - offset;
            fullPointV[nineq + px] = orig - offset;
            double f2 = ff(point, thrId, np, fullPointV, yy_e, rho, vscale_e, a_e, b_e, flag);
            Gaprox[k] = (f1 - f2) / (2.0 * offset);
            offset *= .5;
        }
        point[px] = orig;
...
#pragma omp parallel for num_threads(numThreads)
    for (int px=0; px < int(point.size()); ++px) {
        int thrId = omp_get_thread_num();
        int thrSelect = numThreads==1? -1 : thrId;
        dfn(ff, refFit, thrSelect, &thrPoint.coeffRef(0, thrId), nineq, np, offset, px, numIter, &grid.coeffRef(0,px), &thrFullPointV.coeffRef(0, thrId), yy_e, rho, vscale_e, a_e, b_e, flag);
        for(int m = 1; m < numIter; m++) {                                              // Richardson Step
            for(int k = 0; k < (numIter - m); k++) {
                // NumDeriv Hard-wires 4s for r here. Why?
                grid(k,px) = (grid(k+1,px) * pow(4.0, m) - grid(k,px))/(pow(4.0, m)-1);
                       ...
                       Eigen::MatrixBase<T2> &point, Eigen::MatrixBase<T3> &gradOut)
{
    switch (algo) {
        case GradientAlgorithm_Forward:{
            ...
            gradientImpl_c(ff, numThreads, refFit, point, order, eps, dfn, gradOut);
            break;}
