...
void omxCallAlgebraFitFunction(omxFitFunction *off) {	// TODO: Figure out how to give access to other per-iteration structures.
...
void omxInitAlgebraFitFunction(omxFitFunction* off, SEXP rObj) {
	...
	newObj->algebra = omxNewMatrixFromMxIndex(newptr, off->matrix->currentState);
	if(OMX_DEBUG && off->matrix->currentState->parentState == NULL) {
