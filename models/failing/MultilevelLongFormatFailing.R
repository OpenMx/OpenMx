...
multilevelModel2 <- mxModel("Multilevel_2",
	...
	mxMatrix("Full", 1, 1, values = 1, labels = "data.ID", name = "currentID"),
	mxAlgebra(Rand[currentID,], name = "currentRand"),
    mxMatrix("Full", 2, 2, 
        labels=c(NA,  NA,
                "currentRand[1,1]", NA), 
        free=FALSE, 
    ...
    mxMatrix("Full", nrow=1, ncol=length(manifestVars),
        values=1,
        free=FALSE,
        labels=c("currentRand[1,2]", "currentRand[1,3]"),
