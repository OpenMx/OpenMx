...
mzModel <- mxModel(ntf, name = "MZNTF",
  ...
  cbind(ParChild, ParChild, CvSps, Vp1)), dimnames=list(colnames(mzData),colnames(mzData)),name="expCovMz"),
...
dzModel <- mxModel(ntf, name = "DZNTF",
  ...
  cbind(ParChild, ParChild, Vp1, CvSps),
  cbind(ParChild, ParChild, CvSps, Vp1)), dimnames=list(colnames(dzData),colnames(dzData)),name="expCovDz"),
...
estimate.matrix <- rbind(round(c(estimate[1:5]^2,estimate[6:7]),3))
dimnames(estimate.matrix) <- list(c('OpenMx-Estimated'),c('Var.E','Var.F','Var.A','Var.S','Var.D','Cor.Sps','Var.Phen'))
estimate.matrix
