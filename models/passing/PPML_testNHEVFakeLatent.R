...
# This case is identical to testNHEVCov2 except for the presence of "fake latents",
# moving the error variance for two of the manifest variables off of them directly,
# and in to latents that regress on to those manifests
...
factorModel <- mxModel("Two Factor NHEV",
	  ...
	  mxPath(from='FL_Y', to='Y', arrows=1, value=1, free=FALSE),
	  mxPath(from='FL_Y', arrows=2, value=2, labels='E2'),
	  ...
	  mxPath(from=c('G', 'H'), arrows=2,values=1.0),
