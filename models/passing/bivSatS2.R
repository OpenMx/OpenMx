...
EC1 <- multSatFit1[['S']]@values
LL1 <- mxEvaluate(objective,multSatFit1)
...
EM2 <- multSatFit2[['M']]@values
EC2 <- multSatFit2[['S']]@values
LL2 <- mxEvaluate(objective,multSatFit2)
...
EC3 <- multSatFit3[['expCov']]@values
LL3 <- mxEvaluate(objective,multSatFit3)
...
EM4 <- multSatFit4[['expMean']]@values
EC4 <- multSatFit4[['expCov']]@values
LL4 <- mxEvaluate(objective,multSatFit4)
...
write.table(colMeans(testData),file="mea",row.names=F,quote=F,col.names=F)
