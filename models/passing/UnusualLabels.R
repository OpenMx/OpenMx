...
model[["\\delta A"]]@free[2,1] <- TRUE
model[["\\delta S"]]@free[2,1] <- FALSE
model[["\\delta S"]]@free[1,2] <- FALSE
model[["\\delta S"]]@labels[1,1] <- "apple"
model[["\\delta S"]]@labels[2,2] <- "banana"
...
objective <- mxRAMObjective("\\delta A", "\\delta S", "\\delta F")
