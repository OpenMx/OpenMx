...
factorModel <- mxModel("One Factor", type="RAM",
    ...
    latentVars = latents,
    mxPath(from=latents, to=manifests, free=c(F,T,T,T,T), values=1),
    mxPath(from=manifests, arrows=2, lbound=.0001),
    mxPath(from=latents, arrows=2, free=T, values=1.0),
    mxData(cov(demoOneFactor), type="cov", numObs=500),
    # mxPath(from="one", to=manifests, arrows=1, free=T, values=mean(demoOneFactor)),
    # mxData(demoOneFactor, type="raw"),
    mxMatrix("Iden", nrow=nVars, name="I"),
    mxMatrix("Full", free=F, values=diag(nrow=nManifest, ncol=nVars), name="Eff"),
