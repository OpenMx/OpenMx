...
if(1) {
  omxCheckCloseEnough(max(sapply(bank, function(t) t$condnum['meat'])), 129, 1)

  omxCheckCloseEnough(max(abs(emp$bias)), .0294, .001)
  omxCheckCloseEnough(norm(check.se("sem"), "2"), .54, .01)
  omxCheckCloseEnough(norm(check.se("meat"), "2"), .26, .01)
  omxCheckCloseEnough(norm(check.se("sw"), "2"), .35, .01)
