...
test_that("LMP vs 2PL", {
  ...
  ## 4933.307  
  expect_equal(k0Model@output$fit,twoplModel@output$fit)
...
test_that("LMP k=1", {
  ...
  expect_equal(as.vector(k1PriorModel@output$algebras$itemModel.fitfunction),4929.897,tolerance=1e-3)
  ...
  ## Should be close, but maybe not exact
  expect_equal(as.vector(est),myEst,tolerance=.01)
