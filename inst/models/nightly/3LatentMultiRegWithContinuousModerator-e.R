...
threeLatentOrthogonal <- mxModel("threeLatentOrthogonal",
    ...
    mxPath(from=latents1, to=indicators1, 
           ...
           free=TRUE, values=.2, lbound=.01, ubound=5,
    ...
    mxPath(from="one", to=c(latents), 
           arrows=1, free=TRUE, values=.1, ubound=5,
           labels=meanLabels),
    mxData(observed=latentMultiRegModerated1, type="raw")
    )

diag(threeLatentOrthogonal$S$lbound) <- .01
...
threeLatentOrthogonalOut <- mxRun(mxModel(threeLatentOrthogonalOut, mxComputeNothing()))
