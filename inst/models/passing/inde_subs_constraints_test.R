...
modelACE  <- mxModel( "oneACEo", pars, var1, modelMZ, modelDZ, multi, estVC, ciACE, m1 )
fitACE <- mxRun(modelACE)
( summACE <- summary(fitACE) )
omxCheckTrue(is.integer(summACE$constraints))
omxCheckTrue(all(summACE$constraints == c(1,1)))
omxCheckEquals(names(summACE$constraints)[1], "")
omxCheckEquals(names(summACE$constraints)[2], "MultinomialWithLinearConstraints.indentifying")
