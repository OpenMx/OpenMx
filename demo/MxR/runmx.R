...
runmx <- function(mx.script,mx.location="C:/Program Files/MX/bin/MXE.EXE"){

  
#example:  
#runmx(mx.script="MyScript.mx",mx.location="/usr/local/bin/mxt161i")


  
############ 
#Grab the mx.script, place "!@front" at the top, and write it back out to the working directory
mx.bottom <- suppressWarnings(readLines(mx.script))
mx.tot <- c("!@front",mx.bottom)
mxfile <- paste(mx.script,"R",sep="")
write(mx.tot,file=mxfile)
mxofile <- paste(gsub(".mx","",mx.script),".mxo",sep="") 
############ 
...
#Remove the parts of "x" that break matrix elements into 100 element chunks & recreate "x" s.t. matrices are contiguous
